...
## Phase 6 closure note (2026-05-20T20:57Z)

Mixed result. Most items applied cleanly; one item (pm2 non-root migration) was attempted twice and rolled back both times after triggering a brief origin outage. The app is back on pm2-root and stable.

### Applied + verified

| Finding | Status | Verification |
|:---|:---|:---|
| **L-4 sysctl hardening** | LIVE | `/etc/sysctl.d/99-lockdown.conf` installed: `log_martians=1`, `send_redirects=0`, `tcp_timestamps=0`, plus pinning of `tcp_syncookies=1` and `accept_redirects=0`, `accept_source_route=0`. `sysctl --system` applied. |
| **L-2 pm2-logrotate module** | LIVE | `pm2 install pm2-logrotate` succeeded. Configured `max_size=50M`, `retain=14`, `compress=true`, `rotateInterval='0 0 * * *'`. Module running under pm2 daemon. |
| **L-6 nginx access-log retention** | LIVE | `/etc/logrotate.d/nginx`: `rotate 3` → `rotate 14`. Maintains existing daily/50M-size policy. |
| **L-8 systemctl default target** | LIVE | `systemctl set-default multi-user.target` (was `graphical.target` — irrelevant on a headless server; cleanup). |
| **M-1 `ubuntu` user shell lockdown + drop NOPASSWD** | LIVE | `usermod -s /usr/sbin/nologin ubuntu`; `/etc/sudoers.d/90-cloud-init-users` now has `ubuntu ALL=(ALL) ALL` (password-required, not `NOPASSWD`). Combined with SSH password auth off (Phase 3), the `ubuntu` account is non-actionable. |
| **L-1 auditd + baseline ruleset** | LIVE | `auditd` + `audispd-plugins` installed. `/etc/audit/rules.d/99-lockdown.rules` covers app `.env` / `keys/` / `secure/`, SSH config, nginx config, sudoers, /etc/passwd, /etc/shadow, /etc/group, UFW config, iptables config, audit-config itself. 15 rules loaded. Service enabled + active. |
| **L-5 tmpfs `/tmp` with `nosuid,nodev,noexec`** | QUEUED | Entry added to `/etc/fstab`; activates on next reboot. Not mounted live to avoid disrupting open file descriptors in running processes. |
| **L-3 stale `linux-image-*` cleanup** | NO-OP | `apt-get autoremove --purge -y` ran clean — 0 removed. The old kernels are marked manual and survive autoremove. Lower priority; not blocking lockdown. |

Backups: `/root/lockdown-backups/2026-05-20/phase6/{sysctl.conf, 90-cloud-init-users, logrotate.nginx}.bak`.

### Deferred — pm2 non-root migration (#48)

**Status: deferred.** Two attempts; rolled back each time after the origin returned 502 for ~30s.

**What was tried:**

1. Created `wavemax` system user (no shell login, home `/home/wavemax`).
2. Chowned `/var/www/wavemax/wavemax-affiliate-program` to `wavemax:wavemax` (including `.env` from 600 root:root → 600 wavemax:wavemax).
3. Generated `/etc/systemd/system/pm2-wavemax.service` via `pm2 startup systemd -u wavemax --hp /home/wavemax`.
4. Stopped `pm2-root.service`; started `pm2-wavemax.service`.

**What failed:**

- Attempt 1: pm2-wavemax service hit `Start request repeated too quickly` after 5 fast restarts within systemd's rate-limit window. Daemon was crashing on start.
- Diagnosis: the systemd unit inherited CWD from systemd's default (/), but interactive tests revealed pm2's `child_process.spawn /usr/bin/node` was failing with `EACCES` when CWD was a directory `wavemax` couldn't read (e.g. `/root` when running via `sudo -u wavemax` without explicit `cd`).
- Patched systemd unit to add `WorkingDirectory=/home/wavemax`. Confirmed bare `pm2 ping` worked from a clean `/home/wavemax/.pm2`. Confirmed pm2 resurrect would spawn workers (they thrashed only on port-3000 collision with the still-running pm2-root — expected).
- Attempt 2 (atomic cutover): stopped pm2-root, started pm2-wavemax via systemd. Service still failed; origin returned 502. Root cause unclear from logs — `journalctl -xeu pm2-wavemax` showed the daemon dying repeatedly but without a clear stack trace.

**Suspected root causes (for the next attempt):**

1. The saved `dump.pm2` was generated by root pm2 and may include daemon-context state (file descriptors, /root paths in working directories) that doesn't translate to wavemax cleanly.
2. The ecosystem.config.js doesn't explicitly set `user: 'wavemax'` — pm2 inherits from the daemon's effective UID, but worker startup may have been mid-fork into a root-only resource (e.g. opening a log file under `/var/www/...` whose parent dir hadn't yet been chowned, or a port < 1024 — though port 3000 is unprivileged).
3. The pm2-server-monit + pm2-logrotate modules came from the root daemon's install; they may need to be re-installed under the wavemax daemon.
4. Cluster-mode reload across two different daemons in the same Type=forking systemd unit needs more careful start-order handling.

**Plan for next attempt (when revisited):**

1. Schedule a 5-minute maintenance window with Cloudflare-side maintenance page.
2. Build the wavemax pm2 environment cleanly first: `cd /home/wavemax && sudo -u wavemax bash` interactively, run `pm2 install pm2-server-monit pm2-logrotate`, then `pm2 start /var/www/.../ecosystem.config.js`, confirm port binds. ONLY THEN `pm2 save` to generate a wavemax-native dump.
3. Stop pm2-root, kill the wavemax test daemon, then `systemctl start pm2-wavemax.service` to enter the daemon-managed steady state.
4. Verify the systemd unit can survive a `systemctl restart` cleanly.

**Current state:**

- App back on pm2-root, port 3000 bound by pm2 daemon (PID 118052), 3 wavemax cluster workers + 2 modules — same as before the attempt.
- Files re-chowned to root:root.
- `/etc/systemd/system/pm2-wavemax.service` removed.
- `wavemax` user retained (it's harmless when not running pm2).
- pm2-root.service re-enabled for boot autostart.
- pm2.io agent.json wasn't found at /root/.pm2/agent.json (the dashboard connection may use a different mechanism — Rick's task to confirm).

### Rick-side Phase 6 items

| Item | Notes |
|:---|:---|
| **M-15 rundberglaundry.com registrar transfer-lock** | Log into Ultahost → Domains → rundberglaundry.com → Domain Status → enable `clientTransferProhibited`. The other two laundry domains have it; this one is the outlier. |
| **M-4 pm2.io cloud account MFA** | Visit https://app.pm2.io → Account → Security → Two-Factor Authentication → enable. Confirms the telemetry stream isn't reachable through a password-only login. |

Phase 6 task #88 closed (with pm2 non-root explicitly deferred and notes for the next attempt).
