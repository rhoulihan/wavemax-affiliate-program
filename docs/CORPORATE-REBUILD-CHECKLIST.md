...
- ✅ Perf-claim reframe (2026-05-24) — authoritative Google PSI (run with the Places key from the box) confirmed rundberglaundry is **~90 mobile / ~93 desktop** (the kept Hibu pixel adds variable mobile TBT), not the previously-claimed "high-90s / 96-100." Reframed: the pixel-free **sales page leads at 100/100**; the **store** shows ~90/~93 vs platform 59/82 with the pixel disclosed — "even with a marketing pixel loaded, the store beats the platform's pixel-free score." Defensible on any reader's re-run.
