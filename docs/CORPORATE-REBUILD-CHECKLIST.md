...
- ✅ **Lighthouse Accessibility fixes (2026-05-06).** Closed the three weighted-failure audits flagged on `/about/`: (1) `[aria-*] attributes match roles` — added `role="combobox"` + `aria-haspopup="listbox"` + `aria-label="Language selector"` to every `.wm-lang-switcher` wrapper (4 spots: corporate-chrome.js × 2 instances, franchise-host.html × 2 instances) so the `aria-expanded` state attribute is valid for the element's role; (2) `color-contrast` — bumped chrome b2 nav links (`#wmlnav-nav > a` and `.wmlnav-drop > a`) from `rgba(255,255,255,.85)` to `#ffffff` over the `#1e72ba` background (was 3.79:1 — failed AA; now 4.59:1 — passes); (3) `heading-order` — corporate footer column headings changed from `<h4>` to `<h3>` (CSS selector and JS template both updated) so the page's heading hierarchy is sequentially-descending (h1 → h2 → h3, no skip).
- ✅ **Site-independence sweep (2026-05-06) — Phase A + B + C.** Removed every dependency on `wavemaxlaundry.com` *except* per-franchise location imagery, which is now isolated behind a single config switch.
  - **Brand assets mirrored to `/public/assets/images/brand/`**: 4 favicons (32/180/192/270) + WaveMAX logo. 69 references across 19 HTML files updated from `wavemaxlaundry.com/wp-content/uploads/2026/03/...` to local `/assets/images/brand/...` paths. JSON-LD `"logo"` properties use absolute `https://wavemax.promo/assets/images/brand/logo-wavemax.png` (Schema.org requires absolute URLs).
  - **External page links removed/replaced**: corporate-chrome.js footer "Full Location Map" link to `wavemaxlaundry.com/locations/` deleted (the in-page locations modal already covers this); franchise-host.html + austin-host-mock.html footer Privacy/Terms links repointed to local `/privacy-policy.html` + `/terms-and-conditions.html`; test-operator-scan.html "Production Customer Login" link host-swapped to `wavemax.promo`. JSON-LD `sameAs` knowledge-graph links to `wavemaxlaundry.com` retained (these are entity references, not navigation).
  - **Per-franchise imagery centralized via `public/assets/js/wm-image-config.js`** — exposes `window.WM_LOCATION_IMAGE_BASE` (currently `https://wavemaxlaundry.com/wp-content/uploads/locations`) plus `wmLocationImage(path)` helper and `data-wm-location-image="..."` HTML resolver. **Single switch to migrate to a self-hosted origin later**: change one line in `wm-image-config.js`. Refactored 7 init scripts to use the helper: `austin-about-init.js`, `austin-commercial-init.js`, `austin-contact-init.js`, `austin-landing-init.js`, `austin-self-serve-init.js`, `austin-wdf-init.js`, `corporate-hero-backdrop.js`. `<script src="/assets/js/wm-image-config.js">` added to 18 host pages (load order is config-first so consumers find `window.wmLocationImage` defined).
  - **Inline `og:image` / `twitter:image` in `franchise-host.html`** still reference `wavemaxlaundry.com/...` (Open Graph requires absolute URLs and these are crawl-time metadata, low priority); flagged with TODO comments for the eventual full origin migration.
  - **Inline content-image references** in corporate page `<style>` `url(...)` declarations and `<img src>` attributes (about/testimonials/why-invest/wavemax-vs-zombiemat/virtual-tour/become-a-franchisee/franchise hero backgrounds) intentionally left pointing at `wavemaxlaundry.com/wp-content/uploads/locations/...` — they ARE the location-data dependency the user explicitly wants to keep until an authoritative source replaces it.
