...
- ☐ **Body-content i18n** for `/become-a-franchisee/`, `/about/`, `/testimonials/`, `/why-invest-in-wavemax/`, `/wavemax-vs-zombiemat/`, `/virtual-tour/`, `/faq/` — chrome translates today, body strings need data-i18n markers + en/es/pt/de translations (per project rule: every string ships with translations in same commit).
