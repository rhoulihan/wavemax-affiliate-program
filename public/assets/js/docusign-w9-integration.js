...
(function() {
  ...
  async function handleStartSigning() {
    try {
      ...
      // Option 1: Redirect to DocuSign (recommended for initial implementation)
      window.location.href = data.signingUrl;
      
      // Option 2: Embedded signing (future enhancement)
      // showEmbeddedSigning(data.signingUrl);
      
    } catch (error) {
      console.error('Failed to start W9 signing:', error);
      showError(window.i18next.t('affiliate.dashboard.settings.signingError', 
        'Failed to start W9 signing. Please try again.'));
  ...
  function handleDocuSignMessage(event) {
    ...
    if (message.event === 'signing_complete') {
      ...
      showSuccess(window.i18next.t('affiliate.dashboard.settings.signingComplete',
  ...
  function handleDocuSignReturn(event, envelopeId) {
    switch (event) {
      case 'signing_complete':
        showSuccess(window.i18next.t('affiliate.dashboard.settings.w9SigningComplete',
          'Your W9 form has been signed successfully! It will be reviewed by our team.'));
        break;
      case 'cancel':
        showInfo(window.i18next.t('affiliate.dashboard.settings.w9SigningCancelled',
          'W9 signing was cancelled.'));
        break;
      case 'decline':
        showWarning(window.i18next.t('affiliate.dashboard.settings.w9SigningDeclined',
          'W9 signing was declined.'));
        break;
      case 'session_timeout':
        showWarning(window.i18next.t('affiliate.dashboard.settings.w9SessionTimeout',
  ...
  function updateW9StatusForDocuSign(statusData) {
    ...
    if (statusData.docusignStatus) {
      const docusignStatusMap = {
        'sent': {
          text: window.i18next.t('affiliate.dashboard.settings.docusignSent', 'W9 form sent - awaiting signature'),
          class: 'bg-blue-100 border-blue-400 text-blue-700'
        },
        'delivered': {
          text: window.i18next.t('affiliate.dashboard.settings.docusignDelivered', 'W9 form opened - in progress'),
          class: 'bg-blue-100 border-blue-400 text-blue-700'
        },
        'completed': {
          text: window.i18next.t('affiliate.dashboard.settings.docusignCompleted', 'W9 form signed - under review'),
          class: 'bg-green-100 border-green-400 text-green-700'
        },
        'declined': {
          text: window.i18next.t('affiliate.dashboard.settings.docusignDeclined', 'W9 signing declined'),
          class: 'bg-red-100 border-red-400 text-red-700'
        },
        'voided': {
          text: window.i18next.t('affiliate.dashboard.settings.docusignVoided', 'W9 request cancelled'),
