...
const socialAuthCustomerService = require('../services/socialAuthCustomerService');
...
exports.completeSocialRegistration = async (req, res) => {
  try {
    ...
    const result = await socialAuthAffiliateService.completeSocialRegistration({
      payload: req.body,
      ip: req.ip,
      cryptoWrapper,
      req
    ...
    if (err.isSocialAuthAffiliateError) {
      return res.status(err.status).json({ success: false, message: err.message, ...err.extras });
    }
    logger.error('Social registration error:', err);
...
exports.pollOAuthSession = async (req, res) => {
  try {
    const result = await socialAuthAffiliateService.pollOAuthSession({
      sessionId: req.params.sessionId
    });
    logger.info('Sending OAuth response:', { success: true, result });
    res.json({ success: true, result });
  } catch (err) {
    if (err.isSocialAuthAffiliateError) {
      return res.status(err.status).json({ success: false, message: err.message });
    }
    logger.error('OAuth session polling error:', err);
    res.status(500).json({
      success: false,
...
exports.completeSocialCustomerRegistration = async (req, res) => {
  try {
    ...
    const result = await socialAuthCustomerService.completeSocialCustomerRegistration({
      payload: req.body,
      ip: req.ip,
      cryptoWrapper,
      req
    ...
    if (err.isSocialAuthCustomerError) {
      return res.status(err.status).json({ success: false, message: err.message, ...err.extras });
    }
